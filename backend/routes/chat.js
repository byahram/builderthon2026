...
4. [절대 금지] 답변 내용(summary) 안에 "참고 자료: ...", "Video ID: ...", "Title: ..." 같은 메타데이터를 텍스트로 절대 포함하지 마십시오. 사용자에게는 오직 답변 내용만 보여주어야 합니다. 이런 정보는 JSON의 sources 필드에 이미 포함되어 있습니다.
            ...
            if (note && note.sources && note.sources.length > 0) {
                ...
                const { data: insertedData, error: insertError } = await supabase // Capture data too
                    ...
                    .insert({
                        ...
                        tags: note.tags || [],
                        sources: note.sources // Save full sources array
                    })
